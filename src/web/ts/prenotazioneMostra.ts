...
function prenotazioni_mostra_importaRisorse(filtrato?: boolean) {
...
function prenotazioni_mostra_importaEdifici(risorse, filtrato?: boolean) {
    $.ajax({
...
function prenotazioni_mostra_importaSale(risorse, edifici, filtrato?: boolean) {
    $.ajax({
        ...
        for (let sala of response) {
            let edificioPrenotato: string = edifici.filter(e => e.id == sala.edificioId)[0].nome;
            listaSale.push({
                id: sala.id,
                nome: sala.nome,
...
function prenotazioni_mostra_importaPrenotazioniPerDettaglio(risorse, edifici, sale, filtrato?: boolean) {
    $.ajax({
        url: "http://localhost:42877/api/Prenotazione/",
        type: "GET",
        dataType: "json",
...
function prenotazioni_OperazioniDinamicheTabella(prenotazioni, risorse, edifici, sale, filtrato?: boolean) {
    let table = $('tbody');
...
function prenotazioni_pulsantiDettaglio() {
    $(".pulsante-modale-prenotazione").on("click", function (e) {
        ...
        let dataDescrizione: any = e.target.getAttribute("data-bs-descrizione");
